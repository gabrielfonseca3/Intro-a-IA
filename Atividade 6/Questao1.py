import heapq as hq  # importando a biblioteca para priorqueue.
...
estados_romenia = [
    ...
    {
        "estado": "Drobeta",
        "acoes": [acao("Mehadia", 75), cao("Craiova", 120)],
        "distancia": 242,
    },
    {
        "estado": "Craiova",
    ...
    },
    {
        "estado": "Pitesti",
        "acoes": [
            acao("Rimnicu Vilcea", 97),
            acao("Craiova", 138),
            acao("Bucharest", 101),
        ],
        "distancia": 100,
    },
    {"estado": "Giurgiu", "acoes": [acao("Bucharest", 90)], "distancia": 77},
    {
        "estado": "Bucharest",
        "acoes": [
            acao("Fagaras", 211),
    ...
    {"estado": "Eforie", "acoes": [acao("Hirsova", 86)], "distancia": 161},
    {
        "estado": "Vaslui",
    ...
    {"estado": "Neamt", "acoes": [acao("Iasi", 87)], "distancia": 234},
]
# Editei o código do professor para adicionar as distâncias.
...
class No:
    ...
    def filhos(self, problema):
        espaco_acoes = next(
            e for e in problema.espaco_estados if e["estado"] == self.estado
        )

        resultado = []
        for acao in espaco_acoes["acoes"]:
            filho = No(
                acao["destino"], self.custo + acao["custo"], self, acao["destino"]
            )
...
problema_romenia = Problema(
    estados_romenia, no_arad, lambda no: no.estado == "Bucharest"
)
